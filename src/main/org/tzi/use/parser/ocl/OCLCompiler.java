...
// $Id: OCLCompiler.java 3950 2012-10-31 18:27:29Z lhamann $
...
public class OCLCompiler {
	...
	public static ASTExpression compileExpressionAST(MModel model,
											   String in,
											   String inName,
											   PrintWriter err,
											   VarBindings globalBindings) {

		return compileExpressionAST(
				model,
				new ByteArrayInputStream(in.getBytes()),
				inName,
				err,
	...
	public static ASTExpression compileExpressionAST(MModel model,
											   ...
											   VarBindings globalBindings)
	{
		return OCLCompiler.compileExpressionAST(
				model,
				state,
				new ByteArrayInputStream(input.getBytes()),
				inName,
	...
	public static ASTExpression compileExpressionAST(MModel model,
											   ...
											   PrintWriter err,
											   VarBindings globalBindings) {
		return compileExpressionAST(model, state, in, inName, err, globalBindings, null);
	}
	...
	private static ASTExpression compileExpressionAST(MModel model,
												MSystemState state,
												InputStream in,
												String inName,
												PrintWriter err,
		...
		try {
			// Parse the input expression
			ASTExpression astExpr = parser.expressionOnly();
			ASTExpression astExpr15 = parser.collectionLiteral();
			ASTExpression astExpr2 = parser.objectReference();
			ASTExpression astExpr4 = parser.additiveExpression();
			ASTExpression astExpr5 = parser.equalityExpression();
			ASTExpression astExpr6 = parser.literal();
			ASTExpression astExpr7 = parser.primaryExpression();
			List astExpr8 = parser.idList();
			return astExpr;
//			if (errHandler.errorCount() == 0 ) {
//
//				// Generate code
